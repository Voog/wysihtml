    ...
    EditorContainer.prototype = {
        init: function() {
            ...
            if (this.settings.handler) {
                this.$el.find(this.settings.handler).on('mousedown.editorcontainer', $.proxy(this.handleMouseDown, this));
            } else {
        ...
        handleMouseMove: function (event) {
            ...
            if ($target.closest(this.settings.insertable_into).length) {
                place = this.getClosestBlock($target) || this.getClosestInline($target) || this.getClosestEditableFirstLine($target);
            } else {
        ...
        updateEditablesCache: function() {
            ...
            this.editables = $.map(this.$editables, $.proxy(function(el) {
                var pos = $(el).offset(),
                    $inliners = $(el).children(this.settings.inline_elements);
