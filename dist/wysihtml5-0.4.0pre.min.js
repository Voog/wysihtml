...
t.fail=f;t.warn=function(a){a="Rangy warning: "+a;t.config.alertOnWarn?window.alert(a):typeof window.console!=p&&typeof window.console.log!=p&&window.console.log(a)};({}).hasOwnProperty?t.util.extend=function(a,b){for(var d in b)b.hasOwnProperty(d)&&(a[d]=b[d])}:f("hasOwnProperty not supported");var w=[],A=[];t.init=g;t.addInitListener=function(a){t.initialized?a(t):w.push(a)};var z=[];t.addCreateMissingNativeApiListener=function(a){z.push(a)};t.createMissingNativeApi=function(a){a=a||window;g();
...
function(){})}catch(n){}if(!b.browser.crashesWhenDefineProperty(c))try{var u={get:function(){return d}};e&&(u.set=function(){});Object.defineProperty(a,c,u)}catch(D){}}})})(wysihtml5);
...
else{if(null===p||b.lang.array(k).contains(p))if(v=h.selection.getSelectedNode(),q=g.getParentElement(v,{nodeName:k}),q==h.element&&(q=null),q){h.selection.executeAndRestore(function(){p&&(q=g.renameElement(q,p));if(n){var a=q;a.className?(a.className=a.className.replace(u,""),a.className+=" "+n):a.className=n}});return}h.commands.support(l)?e(D,l,p||s,n):(q=D.createElement(p||s),n&&(q.className=n),f(h,q))}},state:function(a,b,c,d,e){c="string"===typeof c?c.toUpperCase():c;a=a.selection.getSelectedNode();
